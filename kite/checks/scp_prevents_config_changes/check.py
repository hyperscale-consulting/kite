...
def check_scp_prevents_config_changes() -> dict:
    """
    Check if there is an effective SCP that prevents changes to AWS Config.

    This check verifies that:
    1. There is an SCP that denies the following AWS Config actions:
    ...
    Returns:
        A dictionary containing the finding for the SCP prevents AWS Config changes
        check.
    """
    ...
    for scp in root_scps:
        try:
            ...
            if _is_config_deny_scp(content):
...
def _is_config_deny_scp(content: Dict[str, Any]) -> bool:
    ...
    Returns:
        True if the SCP denies the required AWS Config actions
    """
...
check_scp_prevents_config_changes._CHECK_ID = CHECK_ID
check_scp_prevents_config_changes._CHECK_NAME = CHECK_NAME
